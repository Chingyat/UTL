...
TEST_SUITE("vector") {
  ...
  TEST_CASE("assignment operators") {
    ...
    v2 = v1;
    CHECK(
        std::equal(std::begin(v1), std::end(v1), std::begin(v2), std::end(v2)));
    v6 = v3;
    CHECK(
        std::equal(std::begin(v3), std::end(v3), std::begin(v6), std::end(v6)));
    v3 = std::move(v7);
    CHECK(
        std::equal(std::begin(v3), std::end(v3), std::begin(v8), std::end(v8)));
    v8 = std::move(v4);
    CHECK(
        std::equal(std::begin(v5), std::end(v5), std::begin(v8), std::end(v8)));
    v5 = {"1", "2", "3", "4", "5"};
    CHECK(v5.size() == 5);
    v1.assign(v6.begin(), v6.end());
    CHECK(
        std::equal(std::begin(v1), std::end(v1), std::begin(v8), std::end(v8)));
    ...
    CHECK(std::all_of(v2.begin(), v2.end(), [](const auto &x) {
    ...
    CHECK(std::all_of(v3.begin(), v3.end(), [](const auto &x) {
      return x == "12345678901234567890";
    }));
  }
  ...
  TEST_CASE("iterators") {
    ...
    CHECK(
        std::equal(std::begin(a2), std::end(a2), std::rbegin(v), std::rend(v)));
    CHECK(
        std::equal(std::begin(a1), std::end(a1), std::cbegin(v), std::cend(v)));
    CHECK(std::equal(std::begin(a2), std::end(a2), std::crbegin(v),
                     std::crend(v)));
  }
  ...
  template <typename T> struct NonequalAllocator {
    using value_type = T;
    ...
    T *allocate(size_t n) {
      auto ret = operator new[](n * sizeof(T),
  ...
  struct ThrowWhenCopy {
    ...
    [[noreturn]] ThrowWhenCopy(const ThrowWhenCopy &) {
      UTL_THROW(std::runtime_error("ThrowWhenCopy"));
    }
    [[noreturn]] ThrowWhenCopy &operator=(ThrowWhenCopy const &) {
      UTL_THROW(std::runtime_error("="));
    }
