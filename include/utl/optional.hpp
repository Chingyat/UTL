...
template <typename T>
constexpr bool is_nothrow_movable_v = std::is_nothrow_move_constructible_v<T>
    &&std::is_nothrow_move_assignable_v<T>;

template <typename T, typename = void> class optional_base;
...
protected:
  constexpr optional_base() noexcept : m_has_value(false) {}
  ...
  constexpr optional_base(const_reference value) : m_has_value(true) {
    new (&m_data) T(value);
  }

  constexpr optional_base(rvalue_reference value) noexcept(
      std::is_nothrow_move_constructible_v<T>)
      : m_has_value(true) {
  ...
  constexpr optional_base(in_place_t, Args &&... args) : m_has_value(true) {
  ...
  optional_base &
  operator=(optional_base &&rhs) noexcept(is_nothrow_movable_v<T>) {
    if (has_value()) {
      if (rhs.has_value())
        **this = std::move(*rhs);
    } else {
...
template <typename T> class optional : public optional_base<T> {
...
public:
  ...
  template <typename U> constexpr T value_or(U &&default_value) const & {
    return base::has_value() ? *base::data()
