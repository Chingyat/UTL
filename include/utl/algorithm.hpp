...
template <typename Tp,
          typename = std::enable_if_t<std::is_trivially_copyable_v<Tp>>>
inline Tp *copy_backward(const Tp *first, const Tp *last,
                         Tp *output_last) noexcept {
...
template <typename Tp,
          typename V = std::enable_if_t<std::is_trivially_copyable_v<Tp>>>
inline Tp *copy(move_if_noexcept_iterator<Tp *> first,
                move_if_noexcept_iterator<Tp *> last,
                Tp *output_first) noexcept {
  const auto num = last - first;
...
template <typename Tp,
          typename V = std::enable_if_t<std::is_trivially_copyable_v<Tp>>>
inline Tp *copy_backward(move_if_noexcept_iterator<Tp *> first,
                         move_if_noexcept_iterator<Tp *> last, Tp *output_last,
                         V * = nullptr) noexcept {
  auto const ret_val = output_last - (last - first);
...
inline void for_each_impl(InputIterator first, InputIterator last,
                          Callable &&callable, std::input_iterator_tag) {
  while (first != last)
    callable(*first++);
}
...
inline void for_each(InputIterator first, InputIterator last,
                     Callable &&callable) {
  for_each_impl(
      first, last, std::forward<Callable>(callable),
