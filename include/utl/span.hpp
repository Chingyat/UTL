...
namespace detail {
template <typename T, std::size_t E> struct span_storage {
  typedef T element_type;
  T *p_{};
  constexpr static std::size_t s_ = E;

  template <std::size_t Extent = E,
            typename = typename std::enable_if<Extent == 0>::type>
  constexpr span_storage() noexcept {}

  constexpr span_storage(T *p, std::size_t e) : p_(p) { assert(e == E); }
};

template <typename T> struct span_storage<T, dynamic_extent> {
  typedef T element_type;
  T *p_{};
  std::size_t s_{};
};
...
public:
  ...
  struct iterator
      : iterator_wrapper<iterator, value_type, reference, pointer,
    ...
    explicit constexpr iterator(pointer p) noexcept : m_data(p) {}
  };
  struct const_iterator
      : iterator_wrapper<const_iterator, value_type, const_reference,
                         const_pointer, std::random_access_iterator_tag,
                         difference_type> {
    using base_type =
        iterator_wrapper<const_iterator, value_type, const_reference,
                         const_pointer, std::random_access_iterator_tag,
    ...
    explicit constexpr const_iterator(const_pointer p) noexcept : m_data(p) {}
  };
  ...
  template <class Container,
            typename = std::enable_if_t<
                ...
                std::is_convertible<std::remove_pointer_t<decltype(utl::data(
                                        std::declval<Container &>()))> (*)[],
                                    element_type (*)[]>::value>,
            typename = decltype(utl::data(std::declval<Container &>())),
            typename = decltype(utl::size(std::declval<Container &>()))>
  constexpr span(Container &cont) : span(utl::data(cont), utl::size(cont)) {}
  ...
  constexpr pointer data() const noexcept { return s_.p_; }

  constexpr size_type size() const noexcept { return s_.s_; }
...
template <class T, std::size_t N>
span<const byte, std::conditional_t<
                     N == dynamic_extent,
